...
def _get_output_shape(labels, objective='classification'):
    ...
    if (labels.dtype() == int):
        ...
        if (labels.min() == 0 and labels.max() == num_unique - 1):
            return num_unique
    raise ValueError('Please transform the label column into integers of range [0, 1, ... K-1]')
...
def get_builtin_neuralnet(arch):
    ...
    """
    if arch == 'mnist':
        return _builtin_neuralnets._MNIST_NET
    elif arch == 'imagenet':
        return _builtin_neuralnets._IMAGENET_NET
    else:
