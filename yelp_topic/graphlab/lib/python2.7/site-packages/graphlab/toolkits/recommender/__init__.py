...
The input data must be an SFrame with a column containing to user ids, a
column containing to item ids, and optionally a column containing target values
such as movie ratings, etc. When a target is not provided (as is the case in
implicit feedback settings), then a collaborative filtering model based on
item-item similarity is returned. For more details, please see the
documentation for :meth:`graphlab.recommender.create`.
...
User Guide
<http://www.graphlab.com/learn/userguide/index.html#Modeling_data_Recommender_systems>`_
